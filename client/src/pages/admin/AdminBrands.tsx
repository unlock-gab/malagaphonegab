    ...
    <div className="space-y-4" dir={dir}>
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <Label className="text-gray-600 text-sm font-semibold">{t("brand_name")}</Label>
        ...
        <div className="space-y-1.5">
          <Label className="text-gray-600 text-sm font-semibold">{t("brand_slug")}</Label>
      ...
      <div className="space-y-1.5">
        <Label className="text-gray-600 text-sm font-semibold">{t("brand_logo")}</Label>
...
export default function AdminBrands() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/brands"] }); setOpen(false); toast({ title: t("success") }); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/brands"] }); setOpen(false); setEditing(null); toast({ title: t("success") }); },
    onError: () => toast({ title: t("failed"), variant: "destructive" }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/brands/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/brands"] }); toast({ title: t("success") }); },
    onError: () => toast({ title: t("failed"), variant: "destructive" }),
  });
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-black text-gray-900">{t("brands_title")}</h1>
            <p className="text-gray-500 text-xs mt-0.5">{brands.length} {t("nav_brands")}</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 font-semibold">{t("no_brands")}</p>
          </div>
        ) : (
            ...
            {brands.map(brand => (
              <div key={brand.id} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-gray-300 hover:shadow-sm transition-all text-center" data-testid={`card-brand-${brand.id}`}>
                ...
                <div className="flex gap-1.5 mt-3">
                  ...
                  <Button size="sm" variant="outline" onClick={() => { if (confirm(t("confirm_delete"))) deleteMutation.mutate(brand.id); }}
        ...
        <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) setEditing(null); }}>
          <DialogContent className="bg-white border-gray-200 text-gray-900 max-w-md shadow-xl" dir={dir}>
            <DialogHeader className="border-b border-gray-100 pb-3">
              <DialogTitle className="text-gray-900 font-bold">{editing ? t("edit_brand") : t("add_brand")}</DialogTitle>
            </DialogHeader>
            <div className="pt-1">
