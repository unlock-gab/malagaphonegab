  ...
  return (
    <div className="space-y-4" dir={dir}>
      <div className="space-y-1.5">
        <Label className="text-gray-600 text-sm font-semibold">{t("supplier_name")}</Label>
        <Input value={form.name} onChange={e => set("name", e.target.value)}
          className="bg-white border-gray-200 text-gray-900 placeholder:text-gray-400" placeholder={t("supplier_name")} data-testid="input-supplier-name" />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <Label className="text-gray-600 text-sm font-semibold">{t("phone")}</Label>
        ...
        <div className="space-y-1.5">
          <Label className="text-gray-600 text-sm font-semibold">{t("email")}</Label>
      ...
      <div className="space-y-1.5">
        <Label className="text-gray-600 text-sm font-semibold">{t("address")}</Label>
...
export default function AdminSuppliers() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/suppliers"] }); setOpen(false); toast({ title: t("success") }); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/suppliers"] }); setOpen(false); setEditing(null); toast({ title: t("success") }); },
    onError: () => toast({ title: t("failed"), variant: "destructive" }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/suppliers/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/suppliers"] }); toast({ title: t("success") }); },
    onError: () => toast({ title: t("failed"), variant: "destructive" }),
  });
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-black text-gray-900">{t("suppliers_title")}</h1>
            <p className="text-gray-500 text-xs mt-0.5">{suppliers.length} {t("nav_suppliers")}</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 font-semibold">{t("no_suppliers")}</p>
          </div>
        ) : (
              ...
              <div key={sup.id} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-gray-300 hover:shadow-sm transition-all" data-testid={`card-supplier-${sup.id}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <Button size="sm" variant="outline" onClick={() => { if (confirm(t("confirm_delete"))) deleteMutation.mutate(sup.id); }}
                ...
                {sup.notes && <p className="text-gray-500 text-xs mt-2 ms-12">{sup.notes}</p>}
              </div>
            ))}
          </div>
        )}
        ...
        <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) setEditing(null); }}>
          <DialogContent className="bg-white border-gray-200 text-gray-900 max-w-lg shadow-xl" dir={dir}>
            <DialogHeader className="border-b border-gray-100 pb-3">
              <DialogTitle className="text-gray-900 font-bold">{editing ? t("edit_supplier") : t("add_supplier")}</DialogTitle>
            </DialogHeader>
            <div className="pt-1">
