  ...
  return (
    <AdminLayout>
      <div className="space-y-5" dir="rtl">
        <div>
          <h1 className="text-lg font-black text-gray-900">الأرباح وتقسيم الشركاء</h1>
          <p className="text-gray-500 text-xs mt-0.5">
            {filtered.length} طلب مكتمل
            {(dateFrom || dateTo) && <span className="text-blue-600 font-medium"> • مفلتر</span>}
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700 text-sm font-semibold">فلتر الفترة</span>
            </div>
            <div className="flex items-end gap-2">
              <div>
                <Label className="text-gray-500 text-xs block mb-1">من</Label>
              ...
              <div>
                <Label className="text-gray-500 text-xs block mb-1">إلى</Label>
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={thisMonth}
                ...
                data-testid="button-filter-month">هذا الشهر</Button>
              {(dateFrom || dateTo) && (
                <Button size="sm" variant="outline" onClick={clearFilter}
                  className="border-gray-200 text-gray-500 hover:bg-gray-50 text-xs h-8" data-testid="button-filter-clear">مسح</Button>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {isLoading ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />) : <>
            <KPI icon={DollarSign} label="إجمالي الإيرادات" value={formatCurrency(totalRevenue)} sub={`${filtered.length} طلب`} iconBg="bg-emerald-50" iconColor="text-emerald-600" />
            <KPI icon={TrendingUp} label="صافي الربح" value={formatCurrency(totalNetProfit)} sub={`هامش ${margin}%`} iconBg="bg-teal-50" iconColor="text-teal-600" />
            <KPI icon={Users} label="حصة الشريك (⅓)" value={formatCurrency(totalPartner)} sub="33.33% من الربح الصافي" iconBg="bg-pink-50" iconColor="text-pink-600" />
            <KPI icon={Users} label="حصة المالك (⅔)" value={formatCurrency(totalOwner)} sub="66.67% من الربح الصافي" iconBg="bg-indigo-50" iconColor="text-indigo-600" />
          </>}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
            <h3 className="text-gray-800 text-sm font-bold flex items-center gap-2 mb-4 pb-2.5 border-b border-gray-100">
              <BarChart3 className="w-4 h-4 text-blue-600" /> تفصيل الفترة
            </h3>
            {isLoading ? <Skeleton className="h-40 rounded-lg" /> : (
              <div className="space-y-2">
                <div className="flex items-center justify-between p-2.5 bg-emerald-50 border border-emerald-100 rounded-lg text-xs">
                  <span className="text-gray-600 font-medium">الإيرادات</span>
                ...
                <div className="flex items-center justify-between p-2.5 bg-red-50 border border-red-100 rounded-lg text-xs">
                  <span className="text-gray-600 font-medium">تكلفة المنتجات</span>
                ...
                {totalExpenses > 0 && (
                  <div className="flex items-center justify-between p-2.5 bg-orange-50 border border-orange-100 rounded-lg text-xs">
                    <span className="text-gray-600 font-medium">مصاريف مرتبطة</span>
                ...
                <div className="flex items-center justify-between p-2.5 bg-teal-50 border border-teal-200 rounded-lg text-sm">
                  <div>
                    <p className="text-teal-700 font-bold">صافي الربح</p>
                    <p className="text-teal-600/60 text-[10px]">هامش {margin}%</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2.5 bg-pink-50 border border-pink-100 rounded-lg text-center">
                    <p className="text-pink-600 text-[10px] font-semibold mb-1">حصة الشريك ⅓</p>
                  ...
                  <div className="p-2.5 bg-indigo-50 border border-indigo-100 rounded-lg text-center">
                    <p className="text-indigo-600 text-[10px] font-semibold mb-1">حصة المالك ⅔</p>
          ...
          <div className="bg-white border border-gray-100 rounded-xl shadow-sm lg:col-span-2 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h3 className="text-gray-800 text-sm font-bold">سجلات الأرباح التفصيلية ({filtered.length})</h3>
            </div>
            {isLoading ? <div className="p-4"><Skeleton className="h-40 rounded-lg" /></div> :
              filtered.length === 0 ? (
                <div className="text-center py-12 px-4">
                  ...
                  <p className="text-gray-600 text-sm font-semibold">لا توجد سجلات</p>
                  <p className="text-gray-400 text-xs mt-1">
                    {(dateFrom || dateTo) ? "لا توجد طلبات مكتملة في هذه الفترة" : "تُنشأ تلقائياً عند تسليم الطلبات"}
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-100 text-gray-500">
                        ...
                        <th className="text-right p-3 font-semibold hidden md:table-cell">تكلفة المنتج</th>
                        <th className="text-right p-3 font-semibold hidden lg:table-cell">المصاريف</th>
                        <th className="text-right p-3 font-semibold">صافي الربح</th>
                        <th className="text-right p-3 font-semibold hidden sm:table-cell">حصة الشريك</th>
                        <th className="text-right p-3 font-semibold hidden md:table-cell">حصة المالك</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filtered.map(r => {
                      ...
                      <tr className="bg-blue-50 border-t border-blue-100 font-bold text-xs">
                        <td className="p-3 text-gray-700" colSpan={2}>الإجمالي</td>
