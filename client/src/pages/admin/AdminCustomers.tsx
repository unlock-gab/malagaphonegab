...
interface CustomerSummary {
  phone: string; name: string; orderCount: number; totalSpent: number; lastOrderDate: string; wilaya: string;
}
interface Order {
...
function CustomerOrders({ phone }: { phone: string }) {
  ...
  const { data: orders = [], isLoading } = useQuery<Order[]>({
    ...
    queryFn: async () => { const res = await fetch(`/api/customers/${encodeURIComponent(phone)}/orders`); return res.json(); },
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <h4 className="text-gray-600 text-xs font-semibold">{t("order_history")}</h4>
        {isLoading ? (
          Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)
        ) : orders.length === 0 ? (
          <div className="text-center py-8 text-gray-400 text-sm">{t("no_orders_yet")}</div>
        ) : (
          orders.map(o => (
...
export default function AdminCustomers() {
  ...
  const { data: customers = [], isLoading } = useQuery<CustomerSummary[]>({ queryKey: ["/api/customers"] });
  const filtered = customers.filter(c =>
    c.name.toLowerCase().includes(search.toLowerCase()) || c.phone.includes(search) || c.wilaya.includes(search)
  );
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-black text-gray-900 flex items-center gap-2">
              <Users className="w-5 h-5 text-blue-600" /> {t("customers_title")}
            </h1>
            <p className="text-gray-500 text-xs mt-0.5">
              {isLoading ? "..." : customers.length} {t("nav_customers")}
            </p>
          </div>
        </div>
        ...
        {!isLoading && customers.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: t("customers_title"), value: customers.length, icon: Users, color: "text-blue-600", bg: "bg-blue-50" },
              { label: t("orders_count"), value: customers.reduce((s, c) => s + c.orderCount, 0), icon: ShoppingCart, color: "text-violet-600", bg: "bg-violet-50" },
              { label: t("total_spent"), value: (customers.reduce((s, c) => s + c.orderCount, 0) / customers.length).toFixed(1), icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50" },
            ].map(s => (
        ...
        <div className="flex gap-5">
          <div className={`${selectedPhone ? "hidden lg:block lg:w-80 lg:flex-shrink-0" : "flex-1"}`}>
            <div className="relative mb-3">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t("search")}
                className="w-full bg-white border border-gray-200 rounded-xl ps-10 pe-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-100"
            ...
            <div className="space-y-2">
              {isLoading ? (
                ...
                <div className="bg-white border border-gray-200 rounded-xl py-12 text-center">
                  ...
                  <p className="text-gray-500 font-semibold text-sm">{t("no_customers")}</p>
                </div>
              ) : (
                filtered.map(c => (
                  <button key={c.phone} onClick={() => setSelectedPhone(c.phone === selectedPhone ? null : c.phone)}
                    className={`w-full text-start bg-white border rounded-xl p-4 hover:border-blue-200 hover:shadow-sm transition-all ${c.phone === selectedPhone ? "border-blue-300 bg-blue-50/50" : "border-gray-200"}`}
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-gray-400 text-xs flex items-center gap-0.5"><Phone className="w-3 h-3" /> {c.phone}</span>
                            <span className="text-gray-400 text-xs flex items-center gap-0.5"><MapPin className="w-3 h-3" /> {c.wilaya}</span>
                          </div>
                        </div>
                      </div>
                      <span className="text-xs bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full font-bold">{c.orderCount}</span>
                    </div>
                    <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-gray-100">
                      <span className="text-gray-500 text-[10px] flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {t("last_order")}: {formatDate(c.lastOrderDate)}
                      </span>
          ...
          {selectedPhone && selectedCustomer && (
            <div className="flex-1 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <div className="flex items-start justify-between p-5 border-b border-gray-100">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      <span className="text-gray-500 text-sm flex items-center gap-1"><Phone className="w-3.5 h-3.5" /> {selectedCustomer.phone}</span>
                      <span className="text-gray-500 text-sm flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {selectedCustomer.wilaya}</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <a href={whatsappUrl(selectedCustomer.phone)} target="_blank" rel="noopener noreferrer"
                    ...
                    <MessageCircle className="w-3.5 h-3.5" /> {t("contact_via_whatsapp")}
                  </a>
              ...
              <div className="p-5">
                <CustomerOrders phone={selectedPhone} />
              </div>
            </div>
          )}
        </div>
